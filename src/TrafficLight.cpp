    ...
    while (true) {
        ...
        if (timeSinceLastUpdate >= cycleDuration) {
            if (_currentPhase == red) {
                _currentPhase = green;
                std::cout << "The light is now green" << std::endl;
            } else if (_currentPhase == green) {
                _currentPhase = red;
                std::cout << "The light is now red" << std::endl;
            }
            auto sent = std::async(std::launch::async, &MessageQueue<TrafficLightPhase>::send, messageQueue, std::move(_currentPhase));
            sent.wait();
            lastUpdate = std::chrono::system_clock::now();
        }
    }
